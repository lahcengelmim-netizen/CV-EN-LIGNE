...
import { Plus, X, Sparkles, Loader2, Star } from 'lucide-react';
...
export const StepSkills: React.FC<Props> = ({ skills, jobTitle, onChange, lang = 'fr' }) => {
  ...
  const fetchAISuggestions = async () => {
    ...
    try {
      const response = await fetch('/api/ai/suggest-skills', {
        ...
        body: JSON.stringify({ jobTitle, lang })
      });
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h2 className="text-xl font-bold text-slate-900 tracking-tight">5. Compétences Clés</h2>
          <p className="text-xs sm:text-sm text-slate-500 mt-1">
            Listez vos savoir-faire techniques (hard skills) et qualités relationnelles (soft skills).
          </p>
        </div>
        ...
        {jobTitle && (
          <button
            ...
            className="self-start sm:self-auto inline-flex items-center gap-1.5 px-3.5 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl text-xs font-bold shadow-xs hover:from-blue-700 hover:to-indigo-700 transition-colors disabled:opacity-50"
          >
            {loadingAi ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Sparkles className="w-3.5 h-3.5 text-yellow-300" />}
            <span>Suggérer des compétences pour « {jobTitle.slice(0, 20)} »</span>
          </button>
        )}
      </div>
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          className="px-5 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-bold shadow-xs transition-colors flex items-center gap-1"
        >
      ...
      {aiSuggestions.length > 0 && (
        <div className="p-4 bg-blue-50/60 rounded-2xl border border-blue-200 space-y-2">
          <div className="text-xs font-bold text-blue-900 flex items-center gap-1.5">
            ...
            <span>Compétences suggérées par l'IA (cliquez pour ajouter) :</span>
          </div>
          <div className="flex flex-wrap gap-2">
            {aiSuggestions.map((name, i) => (
              <button
                ...
                className="px-3 py-1 bg-white hover:bg-blue-600 hover:text-white text-slate-700 text-xs font-semibold rounded-lg border border-blue-200 shadow-2xs transition-colors flex items-center gap-1"
              >
      ...
      <div className="space-y-3">
        <div className="text-xs font-bold text-slate-700 uppercase tracking-wider">
          Vos compétences sélectionnées ({skills.length})
        </div>

        {skills.length === 0 ? (
          <div className="p-6 text-center bg-slate-50 rounded-2xl border border-slate-200 text-xs text-slate-500">
            Aucune compétence ajoutée. Tapez une compétence ci-dessus ou utilisez les suggestions IA.
          </div>
        ) : (
